...
int recurr(int n,int a,int b,int c,vector<int>& dp)
{
    if(n==0) return 0; //means you cant cut further
    if(n<0) return -1; //this is not possible
    if(dp[n]!=-2) return dp[n];
 
    int recA=recurr(n-a,a,b,c,dp);
    int recC=recurr(n-c,a,b,c,dp);
    int recB=recurr(n-b,a,b,c,dp);
 
    int result=max({recA,recB,recC});
 
