...
void recurrsion(vector<int>& a ,int k,int z,int i,int sum,int& maxi){
    if(k==0){
        maxi=max(maxi,sum);
        return;
    }
    //pointer can go left or right
    //left
    if(z>0 and i>=1){
        sum+=a[i-1];
        maxi=max(maxi,sum);
...
 
        prefix_sum += a[i];
        int moves_left = (k - i) / 2;
 
        if (moves_left <= z) {
            int curr_score = prefix_sum + moves_left * max_pair_sum;
