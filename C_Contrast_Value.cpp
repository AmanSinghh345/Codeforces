...
void recurr(vector<vector<int>>& ans,vector<int>& a,vector<int>& output,int i){
    if(i==a.size()){
        ans.push_back(output);
        return;
    };
    output.push_back(a[i]);
    recurr(ans,a,output,i+1);
    output.pop_back();
    recurr(ans,a,output,i+1);
    return;
}
...
for (auto &sub : ans) {
    ...
    for (int j = 0; j < sub.size() - 1; ++j) {
        conB += abs(sub[j] - sub[j + 1]);
    }

    if (conB == conA) {
        mini = min(mini, (int)sub.size());
    }
}
